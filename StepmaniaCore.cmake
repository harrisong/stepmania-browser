...
# Check for Emscripten first (before other platforms)
if(CMAKE_SYSTEM_NAME STREQUAL "Emscripten" OR EMSCRIPTEN)
  ...
  # Include Emscripten-specific configuration
  if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/emscripten-platform-detection.cmake")
    include("${CMAKE_CURRENT_LIST_DIR}/emscripten-platform-detection.cmake")
  endif()
...
if(WITH_OGG)
  if(WIN32 OR MACOSX OR EMSCRIPTEN)
    # Windows, macOS, and Emscripten use bundled OGG libraries
    # Emscripten provides via -s USE_OGG=1 -s USE_VORBIS=1
    set(HAS_OGG TRUE)
  else()
    find_package(Ogg)
    find_package(Vorbis)
...
if(WITH_SDL)
  if(EMSCRIPTEN)
    # Emscripten provides SDL2 via -s USE_SDL=2
    set(SDL2_FOUND TRUE)
    set(HAS_SDL TRUE)
    message(STATUS "Using Emscripten's SDL2 implementation")
  else()
...
find_package(BZip2)
if(NOT ${BZIP2_FOUND} AND NOT MSVC AND NOT EMSCRIPTEN)
  message(FATAL_ERROR "Bzip2 support required.")
elseif(EMSCRIPTEN)
  # Emscripten: BZip2 not strictly required for browser build
  # Can be added later if needed for compressed assets
  set(BZIP2_FOUND FALSE)
  message(STATUS "BZip2 disabled for Emscripten build")
endif()

find_package(Iconv)

if(EMSCRIPTEN)
  # Emscripten: Threading support is limited and requires special flags
  # For initial build, disable threading
  set(Threads_FOUND FALSE)
  set(HAS_PTHREAD FALSE)
  message(STATUS "Threading disabled for Emscripten initial build")
else()
